...
fn shade_transparent(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
  // odds of absorption inside material
  // if exiting we russian roulette sample absorption
  if(hit.refractive_ratio > 1){
    // cool cyan color glass
    // but next to the green wall it willl look more green than blue
    const ext_coeff = vec3f(0.2, 0.7, 0.8);
    hit.factor = hit.factor * ext_coeff;
    // Tr is probability of transmission
    let Tr = (exp(-hit.factor.x*ext_coeff.x) + exp(-hit.factor.y*ext_coeff.y) + exp(-hit.factor.z*ext_coeff.z)) * 0.333;
    let decision = rnd(seed);
    if(decision > Tr){
      // we didnt get transmission so this is absorption
      return vec3f(0);
    }
    hit.factor = hit.factor / Tr;
  }
