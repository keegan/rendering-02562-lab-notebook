...
async function main() {
  ...
  var frame_num = 0;
  var uniforms_int = new Int32Array([canvas.width, canvas.height, frame_num, shader]);
...
const lightIndicesBuffer = device.createBuffer({
  ...
  const bg_tex = await load_texture(device, "data/luxo_pxr_campus.jpg");
  ...
  entries: [
    ...
    { binding: 13, resource: bg_tex.createView() },
