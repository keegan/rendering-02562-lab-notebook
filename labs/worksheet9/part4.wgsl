...
fn holdout_occlusion(ray: Ray, seed: ptr<function, u32>) -> f32 {
  ...
  let p = aabb.min; // bottom of object
  // ONB is tangent, binormal, normal
  let onb = Onb(
    vec3f(0, 1, 0),
    vec3f(0, 0, 1),
    vec3f(0, 1, 0),
  ...
  // in a cosine-weighted hemisphere above the hit normal
  ...
  // see if we're in shadow from that direction
  if(check_shadow(hit.position, dir, 1e1)){
    return f32(0);
  }


  //get light incoming from sampled direction
// struct Ray {
//   origin: vec3f,
//   direction: vec3f,
//   tmin: f32,
...
fn sample_skybox_light(r: Ray) -> vec3f {
  ...
  let s = texture_linear(bg_texture, uv);
   ...
   // convert from RGBE to RGBA
  let power = s.a;
  let factor = pow(2, (power * 255) - 128);
  let result = s.rgb * factor;
  return result;
}

fn sample_skybox(r: Ray) -> vec3f{
  let s = sample_skybox_light(r);
  // gamma correct 
  //vec4f(pow(accum_color, vec3f(1.0 / uniforms_f.gamma)), 1.0);
  return pow(s, vec3f(1.0 / 1.5));
}
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
  ...
  for(var i = 0; i < max_depth; i ++){
    if(int_scene(&r, &hit)){
      ...
      // sample bg_texture
      // find the point on an imaginary sky box we would hit to find our UV coordinates
      r.tmax = 1e9;
