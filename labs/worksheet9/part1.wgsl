...
fn sample_skybox(r: Ray) -> vec3f{
  let uv = int_skybox(r);
  let bg_texture_sample = texture_linear(bg_texture, uv);
  return bg_texture_sample;
}
...
fn sample_env_light(p: vec3f, normal: vec3f, seed: ptr<function, u32>) -> Light {
  ...
  let Le = 10*sample_skybox(Ray(p, dir, 1e-1, 1e9));
