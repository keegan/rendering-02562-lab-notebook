...
fn int_plane(ray: Ray, hit: ptr<function, HitInfo>, plane_point: vec3f, plane: Onb) -> bool{
  ...
  if(abs(omega_dot_n) > 1e-4){
    ...
    if(ray.tmax >= t && ray.tmin <= t){
      ...
      let texture_color = select(plane_color, texture_linear(grass_texture, (*hit).texcoords), uniforms_int.texture_enabled != 0);
...
fn main_fs(@location(0) coords: vec2f) -> @location(0) vec4f{
  ...
  const max_depth = 10;
  let ipcoords = vec2f(coords.x*uniforms_f.aspect*0.5, coords.y*0.5);
