...
async function main() {
  ...
  let positions = [
    [-0.2, 0.1, 0.9, 1.0], // must be padded to 4 long for storage buffer byte-aligning 
    [0.2, 0.1, 0.9, 1.0], 
    [-0.2, 0.1, -0.1, 1.0]
  ];
  let drawingInfo = new Object();
  drawingInfo.vertices = new Float32Array(positions.flat());
  drawingInfo.indices = new Uint32Array([0, 1, 2, 0]);
