  ...
  if(desc >= 0){
    ...
    // an entry and exit point. calculate both and find which is closer
      ...
      if(ray.tmax >= t1 && ray.tmin <= t1){ // if t1 in bounds its always closer
        ...
        (*hit).position = pos;
        (*hit).normal = normalize(pos - center);
        (*hit).shader = uniforms.glass_shader; // shader 2 = reflective
      }
    }
  }
  return (*hit).hit;
...
fn lambert(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f {
  ...
  var lightray =  Ray((*hit).position, light.wi, 10e-4, light.dist-10e-4);
  var lighthit = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), 0);
  var Lr = ((*hit).color_amb / 3.14159);
  if(!int_scene(&lightray, &lighthit)){
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f{
  switch (*hit).shader {
    ...
    case default {return (*hit).color_diff;}
  }  
}
...
fn main_fs(@location(0) coords: vec2f) -> @location(0) vec4f{
  ...
  var hit = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), 0);
